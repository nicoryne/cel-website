...
import {
  ...
  handleInsert,
  handleUpdate
} from '@/components/admin/clients/league-schedule/utils';
            ...
            <tbody>
              {paginatedSchedules.map((schedule, index) => (
                <tr className="border-b-2 border-neutral-900 text-left" key={index}>
                  ...
                  <td className="px-6 py-3 font-thin">{new Date(schedule.start_date).toLocaleDateString()}</td>
                  <td className="px-6 py-3 font-thin">{new Date(schedule.end_date).toLocaleDateString()}</td>
                  <td className="px-2 py-3">
                    <div className="flex gap-8">
                      <ButtonUpdate
                        onUpdate={() => handleUpdate(setModalProps, formData, schedule, setCachedSchedules)}
                      />
                      <ButtonDelete onDelete={() => handleDelete(setModalProps, schedule, setCachedSchedules)} />
                    </div>
                  </td>
                </tr>
              ))}
