...
import { handleDelete, handleUpdate } from '@/components/admin/clients/players/utils';
...
export default function PlayerCard({ player, setModalProps, formData, setCachedPlayers }: PlayerCardProps) {
  return (
    <div className="flex flex-col rounded-md border-2 border-neutral-700 bg-neutral-900 shadow-lg">
      <div className="flex gap-4 p-4">
        <div className="flex flex-col place-items-center gap-2">
          ...
          <div className="flex place-items-center gap-4">
            <ButtonUpdate onUpdate={() => handleUpdate(setModalProps, formData, player, setCachedPlayers)} />
            <div className="flex gap-1">
              <ButtonDelete onDelete={() => handleDelete(setModalProps, player, setCachedPlayers)} />
            </div>
          </div>
        </div>
      ...
      <footer className="flex justify-between border-t-2 border-neutral-600 bg-neutral-900 px-4 py-2">
        <div className="flex gap-4">
          <Image src={player.team?.logo_url!} alt={`${player.team?.school_abbrev} Logo`} height={16} width={16} />
