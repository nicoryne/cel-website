...
import { addMapToCache, handleInsert } from '@/components/admin/clients/maps/utils';
import { getMapsByIndexRange } from '@/api/maps';
import MapsCard from '@/components/admin/clients/maps/card';
        ...
        <div className="relative overflow-x-auto border-2 border-neutral-800 sm:rounded-lg">
          ...
          <ul className="grid grid-cols-1 gap-8 p-8 sm:grid-cols-2 md:grid-cols-3">
            {paginatedMaps.map((map, index) => (
              <li key={index}>
                <MapsCard map={map} setModalProps={setModalProps} setCachedMaps={setCachedMaps} formData={formData} />
              </li>
            ))}
          </ul>
        </div>
