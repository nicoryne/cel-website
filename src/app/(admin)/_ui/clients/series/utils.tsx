...
import { GamePlatform, LeagueSchedule, Series, SeriesFormType, SeriesWithDetails, Team } from '@/lib/types';
import React from 'react';
import { callModalTemplate } from '@/app/(admin)/_ui/clients/utils';
import { appendSeriesDetails, createSeries, deleteSeriesById, updateSeriesById } from '@/api/series';
import SeriesForm from '@/app/(admin)/_ui/clients/series/form';
...
export const updateSeriesFromCache = (
  ...
  setCachedSeries((prev) => {
    ...
    const updated = prev.map((cachedSeries) => (cachedSeries.id === series.id ? series : cachedSeries));
...
export const handleUpdate = async (
  ...
  const updateExistingSeries = async () => {
    try {
      const updatedSeries: Series | null = await updateSeriesById(series.id, formData.current as SeriesFormType);
      const processedSeries: SeriesWithDetails = appendSeriesDetails(
        platformList,
        teamList,
        leagueScheduleList,
