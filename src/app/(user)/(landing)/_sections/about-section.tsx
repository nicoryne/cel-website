  ...
  return (
    <section aria-labelledby="about-heading" id="about" className="min-h-screen">
      ...
      <div className="p-8">
        ...
        >
          <h2 className="dark:bg-red-blue-gradient bg-blue-red-gradient bg-clip-text text-5xl uppercase text-transparent md:text-6xl">
            {headerTitle}
          </h2>
          <p className="text-wrap text-base md:text-lg">{headerDesc}</p>
        </header>
          ...
          {StatInformation.map((info, index) => (
            <li key={index}>
              <div className="flex h-auto flex-col gap-1 p-4 text-center">
                <h3 className="text-5xl">{info.heading}</h3>
                <span className="text-sm font-semibold md:text-base">{info.desc}</span>
              </div>
            </li>
          ))}
        </ul>
      ...
      <div className="flex flex-col px-8 py-16">
        {AboutUsData.map((data, index) => (
          <MotionComponent
            type="article"
            className="mx-auto grid gap-4 py-8 lg:w-[80vw] lg:grid-cols-2 lg:gap-8"
            key={index}
            variants={{
        ...
        >
          ...
          <div className="relative z-10 flex h-full items-center justify-center">
            <p className="lg:font-2xl font-sm text-center font-delagothic uppercase text-white xl:text-5xl">
